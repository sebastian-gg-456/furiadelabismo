        ...
        if (currentSessionId) {
            ...
            if (sessions[currentSessionId]) {
                const sessionStartTime =
                    sessions[currentSessionId].sessionStartTime || Date.now();
...
if (
    ...
    Phaser.GameObjects.GameObject.prototype.removeAllListeners = function (
        event,
    ) {
        try {
...
export class LoginScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Título
        this.add
            .text(width / 2, 80, "FURIA DE LA ABISMO", {
        ...
        this.add
            .text(width / 2, 140, "Sistema de Sesiones", {
                font: "24px Arial",
                fill: "#00ff00",
            })
            .setOrigin(0.5);
        ...
        if (sessionsList.length > 0) {
            this.add
                .text(width / 2, 200, "Sesiones Guardadas:", {
            ...
            const newSessionButton = this.add
                .text(width / 2, yPos + 40, "+ NUEVA SESIÓN", {
            ...
            this.add
                .text(width / 2, 220, "No hay sesiones guardadas", {
                    font: "18px Arial",
                    fill: "#ff9999",
                })
                .setOrigin(0.5);
...
function ensureBattleMusic(scene) {
    try {
        ...
        if (!game) return;
        // Si ya existe y fue destruida, limpiar referencia
        if (game.battleMusic && game.battleMusic.isDestroyed)
    ...
    try {
        ...
        const addBg = function () {
            try {
                ...
                try {
                    if (scene._bgImage && scene._bgImage.destroy)
                        scene._bgImage.destroy();
                } catch (e) {}
                const img = scene.add
                    .image(camWidth / 2, camHeight / 2, key)
                    .setOrigin(0.5, 0.5)
                    .setDepth(-100);
                try {
                    img.setDisplaySize(camWidth, camHeight);
                } catch (e) {}
...
export class Preloader extends Phaser.Scene {
    ...
    preload() {
        ...
        const mapping = {
            ...
            repo2: {
                ...
                block: "pj2/pj2-disparo.png",
                charge: "pj2/pj2-disparo.png",
                hurt: "pj2/pj2-disparo.png",
                jump: "pj2/pj2-disparo.png",
            },
            repo3: {
                // Franchesca (pj3)
                ...
                robo: "pj3/pj3-ataque2.png",
                block: "pj3/pj3-bloqueo-energia.png",
                charge: "pj3/pj3-bloqueo-energia.png",
            },
            repo4: {
                ...
                block: "pj4/pj4-golpe.png",
                charge: "pj4/pj4-golpe.png",
                hurt: "pj4/pj4-golpe.png",
                jump: "pj4/pj4-golpe.png",
            },
        };
        ...
        Object.keys(mapping).forEach((repoKey, idx) => {
            ...
            for (const action in m) {
                ...
                this.load.spritesheet(key, m[action], {
                    frameWidth: 64,
                    frameHeight: 64,
                });
            }
            ...
            (file) => {
                try {
                    if (file && file.key === "map1" && !this._map1Retry) {
                        console.warn(
                            "map1 failed to load (relative), retrying with absolute path",
                        );
        ...
        const originalOnProcessError =
            this.load.onProcessError || function () {};
        this.load.onProcessError = function (file) {
            if (
                file &&
                file.type === "audio" &&
                (file.key === "menu_music" || file.key === "battle_music")
            ) {
        ...
        this.load.spritesheet("flying_enemy", "enemigo-ataque.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        // Proyectil del enemigo volador (spritesheet 64x64): frames 0-10 vuelo, 10-16 impacto
        this.load.spritesheet("enemy_projectile", "enemigo-disparo.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        // Enemigo terrestre (spritesheet 64x64): frames 0-5 para moverse
        this.load.spritesheet("ground_enemy_walk", "moverce-enemigo2.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        // Enemigo terrestre (spritesheet 64x64): frames 0-5 para atacar/desviar
        this.load.spritesheet("ground_enemy_attack", "ataque-enemigo2.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        // Boss (spritesheets 64x64): caminar (idle=0, mover=1-2), ataque AoE (0-3), muerte (0-1)
        this.load.spritesheet("boss_walk", "jefe-caminar.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        this.load.spritesheet("boss_attack", "ataque-jefe.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        this.load.spritesheet("boss_death", "muerte-jefe.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        // Proyectil del jefe (10 frames, 64x64)
        this.load.spritesheet("boss_projectile", "jefe-disparo.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
    ...
    create() {
        ...
        try {
            if (
                this.textures.exists("sofia_piedra") &&
                !this.anims.exists("sofia_piedra_spin")
            ) {
                const tex = this.textures.get("sofia_piedra");
                let totalFrames = 1;
                try {
                    if (tex && typeof tex.frameTotal === "number")
                        totalFrames = Math.max(1, tex.frameTotal);
                    else if (tex && tex.frames)
                        totalFrames = Math.max(
                            1,
                            Object.keys(tex.frames).filter((k) => !isNaN(+k))
                                .length,
                        );
                } catch (e) {
                    totalFrames = 1;
                ...
                const frames = this.anims.generateFrameNumbers("sofia_piedra", {
                    start: 0,
                    end: endFrame,
                });
                this.anims.create({
                    key: "sofia_piedra_spin",
                    frames,
                    frameRate: 12,
                    repeat: -1,
                });
            ...
            /* ignore piedra anim errors */
        }
        // Animación del proyectil del jefe (frames 0-9)
        try {
            if (
                this.textures.exists("boss_projectile") &&
                !this.anims.exists("boss_projectile_fly")
            ) {
                const framesBoss = this.anims.generateFrameNumbers(
                    "boss_projectile",
                    { start: 0, end: 9 },
                );
                this.anims.create({
                    key: "boss_projectile_fly",
                    frames: framesBoss,
                    frameRate: 16,
                    repeat: -1,
                });
            ...
            /* ignore boss projectile anim errors */
        }
        ...
        try {
            if (this.textures.exists("charles_bullet")) {
                ...
                if (!this.anims.exists("charles_bullet_fly")) {
                    const flyFrames = this.anims.generateFrameNumbers(
                        "charles_bullet",
                        { start: 0, end: flyEnd },
                    );
                    this.anims.create({
                        key: "charles_bullet_fly",
                        frames: flyFrames,
                        frameRate: 16,
                        repeat: -1,
                    });
                ...
                if (!this.anims.exists("charles_bullet_impact")) {
                    ...
                    const impactFrames = this.anims.generateFrameNumbers(
                        "charles_bullet",
                        { start: startImpact, end: endImpact },
                    );
                    this.anims.create({
                        key: "charles_bullet_impact",
                        frames: impactFrames,
                        frameRate: 18,
                        repeat: 0,
                    });
            ...
            if (this.textures.exists("sofia_bullet")) {
                ...
                let totalFramesS = 1;
                try {
                    if (texS && typeof texS.frameTotal === "number")
                        totalFramesS = Math.max(1, texS.frameTotal);
                    else if (texS && texS.frames)
                        totalFramesS = Math.max(
                            1,
                            Object.keys(texS.frames).filter((k) => !isNaN(+k))
                                .length,
                        );
                } catch (e) {
                    totalFramesS = 1;
                ...
                if (!this.anims.exists("sofia_bullet_fly")) {
                    const flyFramesS = this.anims.generateFrameNumbers(
                        "sofia_bullet",
                        { start: 0, end: flyEndS },
                    );
                    this.anims.create({
                        key: "sofia_bullet_fly",
                        frames: flyFramesS,
                        frameRate: 1,
                        repeat: -1,
                    });
                ...
                if (!this.anims.exists("sofia_bullet_impact")) {
                    ...
                    const impactFramesS = this.anims.generateFrameNumbers(
                        "sofia_bullet",
                        { start: startImpactS, end: endImpactS },
                    );
                    this.anims.create({
                        key: "sofia_bullet_impact",
                        frames: impactFramesS,
                        frameRate: 18,
                        repeat: 0,
                    });
                }
            ...
            if (
                this.textures.exists("charles_explosion") &&
                !this.anims.exists("charles_explosion")
            ) {
                ...
                const frames = this.anims.generateFrameNumbers(
                    "charles_explosion",
                    { start: 0, end: endFrame },
                );
                this.anims.create({
                    key: "charles_explosion",
                    frames,
                    frameRate: 18,
                    repeat: 0,
                });
            ...
            if (
                this.textures.exists("mario_agua") &&
                !this.anims.exists("mario_agua")
            ) {
                ...
                const frames = this.anims.generateFrameNumbers("mario_agua", {
                    start: 0,
                    end: endFrame,
                });
                this.anims.create({
            ...
            if (
                this.textures.exists("mario_bola_agua") &&
                !this.anims.exists("mario_bola_agua_fall")
            ) {
            ...
            if (
                this.textures.exists("mario_bola_agua") &&
                !this.anims.exists("mario_bola_agua_impact")
            ) {
                const tex = this.textures.get("mario_bola_agua");
                let totalFrames = 1;
                try {
            ...
            if (
                this.textures.exists("mario_laser_sangre") &&
                !this.anims.exists("mario_laser_sangre")
            ) {
            ...
            try {
                if (this.textures.exists("char1_shoot")) {
                    ...
                    const framesSofia = this.anims.generateFrameNumbers(
                        "char1_shoot",
                        { start: 1, end: endShoot },
                    );
                ...
                if (
                    this.textures.exists("sofia_walk") &&
                    !this.anims.exists("sofia_walk")
                ) {
                    const texWalk = this.textures.get("sofia_walk");
                    let totalWalk = 1;
                    try {
                        if (texWalk && typeof texWalk.frameTotal === "number")
                            totalWalk = Math.max(1, texWalk.frameTotal);
                        else if (texWalk && texWalk.frames)
                            totalWalk = Math.max(
                                1,
                                Object.keys(texWalk.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const framesWalk = this.anims.generateFrameNumbers(
                        "sofia_walk",
                        { start: 0, end: endWalk },
                    );
                    this.anims.create({
                ...
                if (
                    this.textures.exists("sofia_punch") &&
                    !this.anims.exists("sofia_punch")
                ) {
                    const texPunch = this.textures.get("sofia_punch");
                    let totalPunch = 1;
                    try {
                        if (texPunch && typeof texPunch.frameTotal === "number")
                            totalPunch = Math.max(1, texPunch.frameTotal);
                        else if (texPunch && texPunch.frames)
                            totalPunch = Math.max(
                                1,
                                Object.keys(texPunch.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const framesPunch = this.anims.generateFrameNumbers(
                        "sofia_punch",
                        { start: 1, end: endPunch },
                    );
                    this.anims.create({
                        key: "sofia_punch",
                        frames: framesPunch,
                ...
                if (
                    this.textures.exists("sofia_charge") &&
                    !this.anims.exists("sofia_block")
                ) {
                    const framesBlock = this.anims.generateFrameNumbers(
                        "sofia_charge",
                        { start: 0, end: 0 },
                    );
                    this.anims.create({
                ...
                if (
                    this.textures.exists("sofia_charge") &&
                    !this.anims.exists("sofia_charge_anim")
                ) {
                    ...
                    try {
                        if (
                            texCharge &&
                            typeof texCharge.frameTotal === "number"
                        )
                            totalCharge = Math.max(1, texCharge.frameTotal);
                        else if (texCharge && texCharge.frames)
                            totalCharge = Math.max(
                                1,
                                Object.keys(texCharge.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const framesCharge = this.anims.generateFrameNumbers(
                        "sofia_charge",
                        { start: 0, end: endCharge },
                    );
                    this.anims.create({
                        key: "sofia_charge_anim",
                        frames: framesCharge,
                        frameRate: 8,
                        repeat: -1,
                    });
            ...
            try {
                ...
                if (this.textures.exists("char2_walk")) {
                    if (this.anims.exists("char2_walk"))
                        this.anims.remove("char2_walk");
                    const texFranWalk = this.textures.get("char2_walk");
                    let totalFranWalk = 1;
                    try {
                        if (
                            texFranWalk &&
                            typeof texFranWalk.frameTotal === "number"
                        )
                            totalFranWalk = Math.max(1, texFranWalk.frameTotal);
                        else if (texFranWalk && texFranWalk.frames)
                            totalFranWalk = Math.max(
                                1,
                                Object.keys(texFranWalk.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endFranWalk = Math.min(
                        3,
                        Math.max(0, totalFranWalk - 1),
                    );
                    const framesFranWalk = this.anims.generateFrameNumbers(
                        "char2_walk",
                        { start: 0, end: endFranWalk },
                    );
                    this.anims.create({
                        key: "char2_walk",
                        frames: framesFranWalk,
                        frameRate: 6,
                        repeat: -1,
                    });
                }
                ...
                if (this.textures.exists("char2_punch")) {
                    if (this.anims.exists("char2_punch"))
                        this.anims.remove("char2_punch");
                    const texFranPunch = this.textures.get("char2_punch");
                    let totalFranPunch = 1;
                    try {
                        if (
                            texFranPunch &&
                            typeof texFranPunch.frameTotal === "number"
                        )
                            totalFranPunch = Math.max(
                                1,
                                texFranPunch.frameTotal,
                            );
                        else if (texFranPunch && texFranPunch.frames)
                            totalFranPunch = Math.max(
                                1,
                                Object.keys(texFranPunch.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endFranPunch = Math.min(
                        3,
                        Math.max(0, totalFranPunch - 1),
                    );
                    const framesFranPunch = this.anims.generateFrameNumbers(
                        "char2_punch",
                        { start: 0, end: endFranPunch },
                    );
                    this.anims.create({
                        key: "char2_punch",
                        frames: framesFranPunch,
                        frameRate: 14,
                        repeat: 0,
                    });
                }
                ...
                if (
                    this.textures.exists("fran_air_aoe") &&
                    !this.anims.exists("fran_air_aoe")
                ) {
                    ...
                    try {
                        if (
                            texFranAoe &&
                            typeof texFranAoe.frameTotal === "number"
                        )
                            totalFranAoe = Math.max(1, texFranAoe.frameTotal);
                        else if (texFranAoe && texFranAoe.frames)
                            totalFranAoe = Math.max(
                                1,
                                Object.keys(texFranAoe.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endFranAoe = Math.min(
                        3,
                        Math.max(0, totalFranAoe - 1),
                    );
                    const framesAoe = this.anims.generateFrameNumbers(
                        "fran_air_aoe",
                        { start: 0, end: endFranAoe },
                    );
                    this.anims.create({
                        key: "fran_air_aoe",
                        frames: framesAoe,
                ...
                /* ignore franchesca override errors */
            }
            ...
            try {
                ...
                if (this.textures.exists("char0_walk")) {
                    if (this.anims.exists("char0_walk"))
                        this.anims.remove("char0_walk");
                    const texWalk0 = this.textures.get("char0_walk");
                    let totalWalk0 = 1;
                    try {
                        if (texWalk0 && typeof texWalk0.frameTotal === "number")
                            totalWalk0 = Math.max(1, texWalk0.frameTotal);
                        else if (texWalk0 && texWalk0.frames)
                            totalWalk0 = Math.max(
                                1,
                                Object.keys(texWalk0.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const framesWalk0 = this.anims.generateFrameNumbers(
                        "char0_walk",
                        { start: 0, end: endWalk0 },
                    );
                    this.anims.create({
                        key: "char0_walk",
                        frames: framesWalk0,
                ...
                if (this.textures.exists("char0_punch")) {
                    if (this.anims.exists("char0_punch"))
                        this.anims.remove("char0_punch");
                    const texPunch0 = this.textures.get("char0_punch");
                    let totalPunch0 = 1;
                    try {
                        if (
                            texPunch0 &&
                            typeof texPunch0.frameTotal === "number"
                        )
                            totalPunch0 = Math.max(1, texPunch0.frameTotal);
                        else if (texPunch0 && texPunch0.frames)
                            totalPunch0 = Math.max(
                                1,
                                Object.keys(texPunch0.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const framesPunch0 = this.anims.generateFrameNumbers(
                        "char0_punch",
                        { start: 0, end: endPunch0 },
                    );
                    this.anims.create({
                        key: "char0_punch",
                        frames: framesPunch0,
                ...
                if (this.textures.exists("char0_punch_fire")) {
                    if (this.anims.exists("char0_punch_fire"))
                        this.anims.remove("char0_punch_fire");
                    const texPF0 = this.textures.get("char0_punch_fire");
                    let totalPF0 = 1;
                    try {
                        if (texPF0 && typeof texPF0.frameTotal === "number")
                            totalPF0 = Math.max(1, texPF0.frameTotal);
                        else if (texPF0 && texPF0.frames)
                            totalPF0 = Math.max(
                                1,
                                Object.keys(texPF0.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const framesPF0 = this.anims.generateFrameNumbers(
                        "char0_punch_fire",
                        { start: 0, end: endPF0 },
                    );
                    // Mucho más lenta (de 16 -> 3 fps)
                    this.anims.create({
                        key: "char0_punch_fire",
                        frames: framesPF0,
                        frameRate: 3,
                        repeat: 0,
                    });
                ...
                if (this.textures.exists("char0_trans")) {
                    if (!this.anims.exists("char0_trans_punch")) {
                        ...
                        let totalT0 = 1;
                        try {
                            if (texT0 && typeof texT0.frameTotal === "number")
                                totalT0 = Math.max(1, texT0.frameTotal);
                            else if (texT0 && texT0.frames)
                                totalT0 = Math.max(
                                    1,
                                    Object.keys(texT0.frames).filter(
                                        (k) => !isNaN(+k),
                                    ).length,
                                );
                        } catch (e) {
                        ...
                        const framesT0 = this.anims.generateFrameNumbers(
                            "char0_trans",
                            { start: 0, end: endT0 },
                        );
                        this.anims.create({
                            key: "char0_trans_punch",
                            frames: framesT0,
                            frameRate: 14,
                            repeat: 0,
                        });
                ...
                /* ignore charles override errors */
            }
            ...
            try {
                ...
                if (
                    this.textures.exists("mario_punch") &&
                    !this.anims.exists("mario_punch")
                ) {
                    ...
                    try {
                        if (
                            texMarioPunch &&
                            typeof texMarioPunch.frameTotal === "number"
                        )
                            totalMarioPunch = Math.max(
                                1,
                                texMarioPunch.frameTotal,
                            );
                        else if (texMarioPunch && texMarioPunch.frames)
                            totalMarioPunch = Math.max(
                                1,
                                Object.keys(texMarioPunch.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endMarioPunch = Math.min(
                        5,
                        Math.max(1, totalMarioPunch - 1),
                    );
                    const framesMarioPunch = this.anims.generateFrameNumbers(
                        "mario_punch",
                        { start: 1, end: endMarioPunch },
                    );
                    this.anims.create({
                        key: "mario_punch",
                        frames: framesMarioPunch,
                        frameRate: 18,
                        repeat: 0,
                    });
                }
                ...
                if (
                    this.textures.exists("mario_walk") &&
                    !this.anims.exists("mario_walk")
                ) {
                    ...
                    try {
                        if (
                            texMarioWalk &&
                            typeof texMarioWalk.frameTotal === "number"
                        )
                            totalMarioWalk = Math.max(
                                1,
                                texMarioWalk.frameTotal,
                            );
                        else if (texMarioWalk && texMarioWalk.frames)
                            totalMarioWalk = Math.max(
                                1,
                                Object.keys(texMarioWalk.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endMarioWalk = Math.min(
                        2,
                        Math.max(0, totalMarioWalk - 1),
                    );
                    const framesMarioWalk = this.anims.generateFrameNumbers(
                        "mario_walk",
                        { start: 0, end: endMarioWalk },
                    );
                    this.anims.create({
                        key: "mario_walk",
                        frames: framesMarioWalk,
                        frameRate: 4,
                        repeat: -1,
                    });
                }
                ...
                if (
                    this.textures.exists("mario_block") &&
                    !this.anims.exists("mario_block")
                ) {
                    const framesMarioBlock = this.anims.generateFrameNumbers(
                        "mario_block",
                        { start: 0, end: 0 },
                    );
                    this.anims.create({
                        key: "mario_block",
                        frames: framesMarioBlock,
                        frameRate: 1,
                        repeat: 0,
                    });
                }
                ...
                if (
                    this.textures.exists("mario_charge") &&
                    !this.anims.exists("mario_charge_anim")
                ) {
                    ...
                    try {
                        if (
                            texMarioCharge &&
                            typeof texMarioCharge.frameTotal === "number"
                        )
                            totalMarioCharge = Math.max(
                                1,
                                texMarioCharge.frameTotal,
                            );
                        else if (texMarioCharge && texMarioCharge.frames)
                            totalMarioCharge = Math.max(
                                1,
                                Object.keys(texMarioCharge.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endMarioCharge = Math.min(
                        1,
                        Math.max(0, totalMarioCharge - 1),
                    );
                    const framesMarioCharge = this.anims.generateFrameNumbers(
                        "mario_charge",
                        { start: 0, end: endMarioCharge },
                    );
                    this.anims.create({
                        key: "mario_charge_anim",
                        frames: framesMarioCharge,
                        frameRate: 8,
                        repeat: -1,
                    });
                }
            } catch (e) {
                /* ignore mario override errors */
            }
            ...
            try {
                ...
                if (
                    this.textures.exists("flying_enemy") &&
                    !this.anims.exists("flying_enemy_idle")
                ) {
                    const framesIdle = this.anims.generateFrameNumbers(
                        "flying_enemy",
                        { start: 0, end: 0 },
                    );
                    this.anims.create({
                        key: "flying_enemy_idle",
                        frames: framesIdle,
                        frameRate: 1,
                        repeat: -1,
                    });
                ...
                if (
                    this.textures.exists("flying_enemy") &&
                    !this.anims.exists("flying_enemy_shoot")
                ) {
                    ...
                    try {
                        if (
                            texFlyingShoot &&
                            typeof texFlyingShoot.frameTotal === "number"
                        )
                            totalFlyingShoot = Math.max(
                                1,
                                texFlyingShoot.frameTotal,
                            );
                        else if (texFlyingShoot && texFlyingShoot.frames)
                            totalFlyingShoot = Math.max(
                                1,
                                Object.keys(texFlyingShoot.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endFlyingShoot = Math.min(
                        1,
                        Math.max(0, totalFlyingShoot - 1),
                    );
                    const framesShoot = this.anims.generateFrameNumbers(
                        "flying_enemy",
                        { start: 0, end: endFlyingShoot },
                    );
                    this.anims.create({
                        key: "flying_enemy_shoot",
                        frames: framesShoot,
                        frameRate: 10,
                        repeat: 0,
                    });
                ...
                if (
                    this.textures.exists("enemy_projectile") &&
                    !this.anims.exists("enemy_projectile_fly")
                ) {
                    const framesFly = this.anims.generateFrameNumbers(
                        "enemy_projectile",
                        { start: 0, end: 10 },
                    );
                    this.anims.create({
                        key: "enemy_projectile_fly",
                        frames: framesFly,
                        frameRate: 16,
                        repeat: -1,
                    });
                ...
                if (
                    this.textures.exists("enemy_projectile") &&
                    !this.anims.exists("enemy_projectile_impact")
                ) {
                    const framesImpact = this.anims.generateFrameNumbers(
                        "enemy_projectile",
                        { start: 10, end: 15 },
                    );
                    this.anims.create({
                        key: "enemy_projectile_impact",
                        frames: framesImpact,
                        frameRate: 16,
                        repeat: 0,
                    });
                ...
                if (
                    this.textures.exists("ground_enemy_walk") &&
                    !this.anims.exists("ground_enemy_walk")
                ) {
                    const texGroundWalk =
                        this.textures.get("ground_enemy_walk");
                    let totalGroundWalk = 1;
                    try {
                        if (
                            texGroundWalk &&
                            typeof texGroundWalk.frameTotal === "number"
                        )
                            totalGroundWalk = Math.max(
                                1,
                                texGroundWalk.frameTotal,
                            );
                        else if (texGroundWalk && texGroundWalk.frames)
                            totalGroundWalk = Math.max(
                                1,
                                Object.keys(texGroundWalk.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endGroundWalk = Math.min(
                        5,
                        Math.max(0, totalGroundWalk - 1),
                    );
                    const framesGroundWalk = this.anims.generateFrameNumbers(
                        "ground_enemy_walk",
                        { start: 0, end: endGroundWalk },
                    );
                    this.anims.create({
                        key: "ground_enemy_walk",
                        frames: framesGroundWalk,
                        frameRate: 10,
                        repeat: -1,
                    });
                }
                ...
                if (
                    this.textures.exists("ground_enemy_attack") &&
                    !this.anims.exists("ground_enemy_attack")
                ) {
                    const texGroundAttack = this.textures.get(
                        "ground_enemy_attack",
                    );
                    ...
                    try {
                        if (
                            texGroundAttack &&
                            typeof texGroundAttack.frameTotal === "number"
                        )
                            totalGroundAttack = Math.max(
                                1,
                                texGroundAttack.frameTotal,
                            );
                        else if (texGroundAttack && texGroundAttack.frames)
                            totalGroundAttack = Math.max(
                                1,
                                Object.keys(texGroundAttack.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endGroundAttack = Math.min(
                        5,
                        Math.max(0, totalGroundAttack - 1),
                    );
                    const framesGroundAttack = this.anims.generateFrameNumbers(
                        "ground_enemy_attack",
                        { start: 0, end: endGroundAttack },
                    );
                    this.anims.create({
                        key: "ground_enemy_attack",
                        frames: framesGroundAttack,
                        frameRate: 18,
                        repeat: 0,
                    });
                }
                ...
                if (this.textures.exists("boss_walk")) {
                    ...
                    if (!this.anims.exists("boss_idle")) {
                        const framesIdleBoss = this.anims.generateFrameNumbers(
                            "boss_walk",
                            { start: 0, end: 0 },
                        );
                        this.anims.create({
                            key: "boss_idle",
                            frames: framesIdleBoss,
                            frameRate: 1,
                            repeat: -1,
                        });
                    }
                    ...
                    if (!this.anims.exists("boss_move")) {
                        ...
                        try {
                            if (
                                texBossWalk &&
                                typeof texBossWalk.frameTotal === "number"
                            )
                                totalBossWalk = Math.max(
                                    1,
                                    texBossWalk.frameTotal,
                                );
                            else if (texBossWalk && texBossWalk.frames)
                                totalBossWalk = Math.max(
                                    1,
                                    Object.keys(texBossWalk.frames).filter(
                                        (k) => !isNaN(+k),
                                    ).length,
                                );
                        } catch (e) {
                        ...
                        const endBossMove = Math.min(
                            2,
                            Math.max(1, totalBossWalk - 1),
                        );
                        const framesBossMove = this.anims.generateFrameNumbers(
                            "boss_walk",
                            { start: 1, end: endBossMove },
                        );
                        this.anims.create({
                            key: "boss_move",
                            frames: framesBossMove,
                            frameRate: 6,
                            repeat: -1,
                        });
                    }
                ...
                if (
                    this.textures.exists("boss_attack") &&
                    !this.anims.exists("boss_attack_anim")
                ) {
                    ...
                    try {
                        if (
                            texBossAtk &&
                            typeof texBossAtk.frameTotal === "number"
                        )
                            totalBossAtk = Math.max(1, texBossAtk.frameTotal);
                        else if (texBossAtk && texBossAtk.frames)
                            totalBossAtk = Math.max(
                                1,
                                Object.keys(texBossAtk.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endBossAtk = Math.min(
                        3,
                        Math.max(0, totalBossAtk - 1),
                    );
                    const framesBossAtk = this.anims.generateFrameNumbers(
                        "boss_attack",
                        { start: 0, end: endBossAtk },
                    );
                    this.anims.create({
                        key: "boss_attack_anim",
                        frames: framesBossAtk,
                        frameRate: 4,
                        repeat: 0,
                    });
                }
                ...
                if (
                    this.textures.exists("boss_death") &&
                    !this.anims.exists("boss_death")
                ) {
                    ...
                    try {
                        if (
                            texBossDeath &&
                            typeof texBossDeath.frameTotal === "number"
                        )
                            totalBossDeath = Math.max(
                                1,
                                texBossDeath.frameTotal,
                            );
                        else if (texBossDeath && texBossDeath.frames)
                            totalBossDeath = Math.max(
                                1,
                                Object.keys(texBossDeath.frames).filter(
                                    (k) => !isNaN(+k),
                                ).length,
                            );
                    } catch (e) {
                    ...
                    const endBossDeath = Math.min(
                        1,
                        Math.max(0, totalBossDeath - 1),
                    );
                    const framesBossDeath = this.anims.generateFrameNumbers(
                        "boss_death",
                        { start: 0, end: endBossDeath },
                    );
                    this.anims.create({
                        key: "boss_death",
                        frames: framesBossDeath,
                        frameRate: 6,
                        repeat: 0,
                    });
                }
            } catch (e) {
                /* ignore enemy anim errors */
            }
        } catch (e) {
            /* ignore charles bullet anim errors */
        }
    ...
    createAnimations() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            actions.forEach((act) => {
                ...
                try {
                    console.log(
                        `ANIM CREATED: ${key} total=${totalFrames} fr=${frameRate} rep=${repeat}`,
                    );
                } catch (e) {}
            });
        }
...
export class GameScene extends Phaser.Scene {
        ...
        try {
            ...
            if (needRecreate) {
                ...
                if (s.body && s.body.setSize)
                    s.body.setSize(40, 56).setOffset(12, 8);
                try {
                    s.setFrame(0);
                } catch (e) {}
                p.sprite = s;
                if (this.mode === "cooperativo" && this.players[1])
                    this.players[1].sprite = s;
            }
            // common sanity
            p.immobilized = false;
            p.blocking = false;
            ...
            if (s.body) {
                try {
                    ...
                    if (s.body.checkCollision) {
                        ...
                        s.body.checkCollision.up =
                            s.body.checkCollision.down =
                            s.body.checkCollision.left =
                            s.body.checkCollision.right =
                                true;
                    }
                } catch (e) {}
            }
            ...
            try {
                if (
                    this.enemies &&
                    this.enemies.contains &&
                    this.enemies.contains(s)
                )
                    this.enemies.remove(s, false, false);
            } catch (e) {}
            // Collide with ground
            try {
            ...
            try {
                if (this._bossSpikesOverlap) {
                    this.physics.world.removeCollider(this._bossSpikesOverlap);
                    this._bossSpikesOverlap = null;
                }
                if (this.bossSpikes && this.players[0].sprite) {
                    this._bossSpikesOverlap = this.physics.add.overlap(
                        this.bossSpikes,
                        this.players[0].sprite,
                        (spike, sprite) => {
                            try {
            ...
            if (this.reticle) {
                try {
                    if (this.children && this.children.bringToTop)
                        this.children.bringToTop(this.reticle);
                } catch (e) {}
                try {
                    if (this.reticle.setDepth) this.reticle.setDepth(11);
                } catch (e) {}
            }
        } catch (e) {
            /* ignore */
    ...
    getEnergy(player) {
        if (this.mode === "cooperativo")
            return this.sharedEnergy != null ? this.sharedEnergy : 0;
    ...
    createMapPlatforms(mapName, width, height) {
        ...
        if (mapName === "Mapa 2" && this._tilemapSueloLayer) {
            console.log(
                "Mapa 2 usa tilemap con texturas, no se crean plataformas invisibles",
            );
            return;
        }
    ...
    buildPlatformsFromData() {
        ...
        try {
            if (this._playerColliders && this._playerColliders.length) {
                this._playerColliders.forEach((c) => {
                    try {
                        this.physics.world.removeCollider(c);
                    } catch (e) {}
                });
            ...
            if (this.players && this.players[1] && this.players[1].sprite)
                this._playerColliders.push(
                    this.physics.add.collider(
                        this.players[1].sprite,
                        this.groundGroup,
            ...
            /* ignore collider refresh errors */
        }
    ...
    create() {
        ...
        try {
            const fs =
                this.scale && this.scale.startFullscreen
                    ? this.scale.startFullscreen()
                    : null;
            if (
                fs &&
                typeof fs.then === "function" &&
                typeof fs.catch === "function"
            ) {
                fs.catch((err) => {
                    /* ignore permission errors like "not granted" */
                });
            }
        } catch (e) {
        ...
        if (this.selectedMap === "Mapa 2") {
            ...
            if (
                this.cache.tilemap.has("mapa2") &&
                this.textures.exists("tileset_mapa2")
            ) {
                try {
                    ...
                    if (tileset) {
                        ...
                        // Capa de decoración
                        try {
                            const decoracionLayer = map.createLayer(
                                "decoracion",
                                tileset,
                                0,
                                0,
                            ...
                            console.warn(
                                "No se pudo crear capa decoracion:",
                                e,
                            );
                        }
                        ...
                        try {
                            const sueloLayer = map.createLayer(
                                "suelo",
                                tileset,
                                0,
                                0,
                            ...
                            if (sueloLayer) {
                                ...
                                console.log(
                                    "Capa de suelo creada con colisiones",
                                );
                            }
                        } catch (e) {
                        ...
                        console.log(
                            "Mapa 2 con texturas cargado correctamente",
                        );
                    } else {
        ...
        this.input.keyboard.on("keydown-[", () => {
            this.input.keyboard.emit("keydown-OPEN_BRACKET");
        });
        this.input.keyboard.on("keydown-]", () => {
            this.input.keyboard.emit("keydown-CLOSE_BRACKET");
        });

        this._bgOffsetText = this.add
            .text(10, 10, `bg offset: ${this._bgYOffset || 0}`, {
                font: "16px Arial",
                color: "#ffffff",
            })
            .setDepth(1000)
        ...
        if (typeof this._platformsVisible !== "boolean")
            this._platformsVisible = false;
        this.createMapPlatforms(this.selectedMap, width, height);

        // Toggle platforms visibility on 'U'
        try {
            this.input.keyboard.on("keydown-U", () =>
                this.togglePlatformsVisibility(),
            );
        } catch (e) {
            /* ignore */
        }
        ...
        if (!this.textures.exists(p1KeyBase)) {
            console.error(
                `Texture ${p1KeyBase} does not exist. Returning to character selector.`,
            );
            this.scene.stop();
        ...
        if (!this.textures.exists(p2KeyBase)) {
            console.error(
                `Texture ${p2KeyBase} does not exist. Returning to character selector.`,
            );
            this.scene.stop();
            this.scene.start("CharacterSelector", { mode: this.mode });
            return;
        ...
        const p2Sprite = this.physics.add
            .sprite(width - 200, spawnY, p2KeyBase)
            .setCollideWorldBounds(true);
            ...
            try {
                if (this.textures.exists("target")) {
                    this.reticle = this.add.image(
                        ...
                        this.players[0].sprite.y + 48,
                        "target",
                    );
                } else {
                    this.reticle = this.add.circle(
                        this.players[0].sprite.x,
                        this.players[0].sprite.y + 48,
                        8,
                        0xffff00,
        ...
        try {
            this.input.keyboard.on("keydown-T", () => {
                ...
                if (this.waveText && this.waveText.setText)
                    this.waveText.setText("¡Jefe final! (debug)");
        ...
        if (this.mode === "cooperativo") {
            ...
            this.physics.add.overlap(
                ...
                (a, b) => {
                    ...
                    const proj =
                        a && a.texture && a.texture.key === "tex_bullet"
                            ? a
                            : b;
                    ...
                    const justBlocked =
                        p.blockPressTime &&
                        now - p.blockPressTime < parryWindow;
                    if (p.blocking && justBlocked && !proj.parried) {
                        ...
                        if (proj.body) {
                            ...
                            if (proj.body.setGravity)
                                proj.body.setGravity(0, 0);
                        ...
                        if (proj.body) {
                            ...
                            proj.body.setDrag(0, 0);
                            if (
                                proj.body.velocity.length &&
                                proj.body.velocity.length() < 10
                            ) {
            ...
            this.physics.add.overlap(
                ...
                (proj, enemy) => {
                    ...
                    if (
                        ...
                        if (
                            enemy.texture &&
                            enemy.texture.key === "ground_enemy_walk" &&
                            this.anims.exists("ground_enemy_attack")
                        ) {
                        ...
                        const playerSprite =
                            this.players[0] && this.players[0].sprite;
                        if (playerSprite && proj.body) {
                            ...
                            const curSpeed = Math.max(
                                300,
                                Math.hypot(curVx, curVy),
                            );
                            proj.body.setVelocity(
                                Math.cos(ang) * curSpeed,
                                Math.sin(ang) * curSpeed,
                            );
                            proj.rotation = ang;
                    ...
                    if (enemy === this.boss) {
                        enemy.health -= damage;
                        console.log(
                            "Boss health after parry hit:",
                            enemy.health,
                        );
                    } else {
                        enemy.health -= damage;
                        console.log(
                            "Enemy health after parry hit:",
                            enemy.health,
                        );
                    }
                    proj.destroy();
    ...
    onProjectileHit(proj, hitPlayerIndex) {
        ...
        if (
            ...
            return; // soporta bala de Charles y Sofía
        ...
        if (target.blocking) {
            ...
            if (!proj.piercing) {
                if (
                    key === "charles_bullet" &&
                    this.anims.exists("charles_bullet_impact") &&
                    !proj._impacting
                ) {
                    ...
                    try {
                        proj.play("charles_bullet_impact");
                    } catch (e) {}
                    proj.once(
                    ...
                    key === "sofia_bullet" &&
                    this.anims.exists("sofia_bullet_impact") &&
                    !proj._impacting
                ) {
                    proj._impacting = true;
                    try {
        ...
        if (this.mode === "cooperativo") {
            const shooterSprite =
                this.players[proj.shooter] && this.players[proj.shooter].sprite;
    ...
    handleProjectilePlayerOverlap(a, b, hitPlayerIndex) {
        ...
        const isProjA =
            a &&
            a.texture &&
            (a.texture.key === "tex_bullet" ||
                a.texture.key === "charles_bullet" ||
                a.texture.key === "sofia_bullet");
        const isProjB =
            b &&
            b.texture &&
            (b.texture.key === "tex_bullet" ||
                b.texture.key === "charles_bullet" ||
                b.texture.key === "sofia_bullet");
        if (isProjA) proj = a;
    ...
    applyDamageToPlayer(targetIndex, damage) {
        ...
        if (this.mode === "cooperativo") {
            if (target.health > 0) {
                ...
                target.secondHealth = Math.max(
                    0,
                    (target.secondHealth || 0) - damage,
                );
                target.beingHit = true;
                target.hitTimer = this.time.now + 300;
            }
    ...
    update(time) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (p.blockingDamageEnd && time < p.blockingDamageEnd) {
                ...
                p.health = Math.max(
                    0,
                    p.health - (p.blockingDamagePerSecond || 0) * dt,
                );
            } else {
            ...
            if (this.mode === "cooperativo") {
                ...
                if (p0 && (p0.secondHealth || 0) <= 0) {
                    ...
                    try {
                        ...
                        console.warn(
                            "Failed to launch GameOver scene (coop):",
                            e,
                        );
                    }
                    try {
                        ...
                        console.warn(
                            "Failed to stop current scene after launching GameOver (coop):",
                            e,
                        );
                    }
                }
                ...
                if (
                    ...
                    try {
                        ...
                        console.warn("Failed to launch GameOver scene:", e);
                    }
                    try {
                        this.scene.stop();
                    } catch (e) {
                        console.warn(
                            "Failed to stop current scene after launching GameOver:",
                            e,
                        );
                    }
                }
        ...
        if (this.mode === "cooperativo") {
            ...
            try {
                ...
                if (this.smallHPBar)
                    this.smallHPBar.width = Math.max(
                        0,
                        (sec / maxSec) * this.smallHPBar.max,
                    );
            } catch (e) {
                /* ignore UI errors */
            }
        } else {
            this.hpBars[1].width = Math.max(
                0,
                (this.players[1].health / maxHP) * barLength,
            );
            this.enBars[1].width = Math.max(
                0,
                (this.players[1].energy / maxEN) * barLength,
            );
        }
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (
                p.franchescaStolenAbility &&
                time > p.franchescaStolenAbility.timer
            ) {
            ...
            for (const abilityName in p.stolenAbilitiesTimers) {
                if (
                    p.stolenAbilitiesTimers[abilityName] &&
                    time > p.stolenAbilitiesTimers[abilityName]
                ) {
                    ...
                    if (
                        p.sprite &&
                        Object.keys(p.stolenAbilities).length === 0
                    ) {
    ...
    updatePlayerInput(i, time) {
        ...
        if (player.shootOverlay && player.shootOverlay.active) {
            try {
                ...
                if (player.shootOverlay.setDepth)
                    player.shootOverlay.setDepth((sprite.depth || 0) + 1);
            } catch (e) {
                /* ignore */
            }
        }
        ...
        if (player.walkOverlay && player.walkOverlay.active) {
            try {
                ...
                if (player.walkOverlay.setDepth)
                    player.walkOverlay.setDepth((sprite.depth || 0) + 1);
            } catch (e) {
                /* ignore */
            }
        }
        ...
        if (player.punchOverlay && player.punchOverlay.active) {
            try {
                ...
                if (player.punchOverlay.setDepth)
                    player.punchOverlay.setDepth((sprite.depth || 0) + 1);
            } catch (e) {
                /* ignore */
            }
        }
        ...
        if (player.blockOverlay && player.blockOverlay.active) {
            try {
                ...
                if (player.blockOverlay.setDepth)
                    player.blockOverlay.setDepth((sprite.depth || 0) + 1);
            } catch (e) {
                /* ignore */
            }
        }
        ...
        if (player.chargeOverlay && player.chargeOverlay.active) {
            try {
                ...
                if (player.chargeOverlay.setDepth)
                    player.chargeOverlay.setDepth((sprite.depth || 0) + 1);
            } catch (e) {
                /* ignore */
            }
        }
        ...
        if (player.beingHit) {
            ...
            if (
                this.anims.exists(hurtKey) &&
                sprite.anims.currentAnim?.key !== hurtKey
            ) {
        ...
        if (this.mode === "cooperativo") {
            ...
            const p1Block =
                ...
                p1Pad.connected &&
                p1Pad.buttons[0] &&
                p1Pad.buttons[0].pressed;
            const p2Block =
                p2Pad &&
                p2Pad.connected &&
                p2Pad.buttons[0] &&
                p2Pad.buttons[0].pressed;
            if (p1Block && p2Block) {
        ...
        if (this.mode === "cooperativo" && i === 1) {
            ...
            if (pad && pad.connected) {
                ...
                const quickPressed =
                    btn[1] && btn[1].pressed && !pad._lastButtons[1];
                const chargeHeld = btn[2] && btn[2].pressed;
                const chargeReleased =
                    (!btn[2] || !btn[2].pressed) && pad._lastButtons[2];
                pad._lastButtons = btn.map((b) => !!b.pressed);

                // Disparo cargado
                if (chargeHeld && this.getEnergy(coopPlayer) > 0) {
                    if (!coopPlayer.chargingShot) {
                        coopPlayer.chargingShot = true;
                        coopPlayer.chargeAmount = 0;
                        coopPlayer.chargeStart = time;
                    }
                    const dt = this.game.loop.delta / 1000;
                    const energyRate = 160;
                    const consume = Math.min(
                        this.getEnergy(coopPlayer),
                        energyRate * dt,
                    );
                    this.changeEnergyFor(coopPlayer, -consume);
                    coopPlayer.chargeAmount =
                        (coopPlayer.chargeAmount || 0) + consume;
                    try {
                        if (this.reticle)
                            this.reticle.setScale(
                                1 +
                                    Math.min(
                                        1.2,
                                        coopPlayer.chargeAmount / 240,
                                    ),
                            );
                    } catch (e) {}
                }
                ...
                if (
                    chargeReleased &&
                    coopPlayer.chargingShot &&
                    coopPlayer.chargeAmount > 0
                ) {
                    const baseDamage = 20;
                    const steps = Math.floor(
                        (coopPlayer.chargeAmount || 0) / 25,
                    );
                    const damage = baseDamage + steps * 3;
                    try {
                        if (this.reticle) this.reticle.setScale(1);
                    } catch (e) {}
        ...
        if (i === 0) {
            ...
            if (!(this.mode === "cooperativo" && i === 1)) {
                if (Phaser.Input.Keyboard.JustDown(this.keysP2.shoot))
                    shoot = true;
            }
        }
        ...
        if (blockOrCharge) {
            ...
            if (
                (isShooterInCoop || this.mode !== "cooperativo") &&
                !player.wasBlocking
            ) {
            ...
            if (dist > chargeDistance) {
                ...
                if (
                    ...
                    const chargeTexture =
                        charIndexLocal === 1 ? "sofia_charge" : "mario_charge";
                    const chargeAnim =
                        charIndexLocal === 1
                            ? "sofia_charge_anim"
                            : "mario_charge_anim";
                    ...
                    if (!player.chargeOverlay) {
                        const chargeSprite = this.add
                            .sprite(sprite.x, sprite.y, chargeTexture, 0)
                ...
                if (
                    ...
                    const blockTexture =
                        charIndexLocal === 1 ? "sofia_charge" : "mario_block";
                    const blockAnim =
                        charIndexLocal === 1 ? "sofia_block" : "mario_block";

                    if (!player.blockOverlay) {
                        const blockSprite = this.add
                            .sprite(sprite.x, sprite.y, blockTexture, 0)
                            .setDepth(sprite.depth + 1);
            ...
            if (this.textures.exists(idleKey)) {
                try {
                    ...
                    if (charIndex === 2)
                        sprite.setFrame(3); // Franchesca idle usa frame 3 de caminar
        ...
        if (
            (charIndex === 0 &&
                this.textures.exists("char0_walk") &&
                this.anims.exists("char0_walk")) ||
            (charIndex === 1 &&
                this.textures.exists("sofia_walk") &&
                this.anims.exists("sofia_walk")) ||
            (charIndex === 2 &&
                this.textures.exists("char2_walk") &&
                this.anims.exists("char2_walk")) ||
            (charIndex === 3 &&
                this.textures.exists("mario_walk") &&
                this.anims.exists("mario_walk"))
        ) {
            ...
            const blockCharlesWalk =
                charIndex === 0 && player.transformed === true;
            if (isMoving && !player.walkOverlay && !blockCharlesWalk) {
                let walkTexture, walkAnim;
                if (charIndex === 0) {
                    walkTexture = "char0_walk";
                    walkAnim = "char0_walk";
                } else if (charIndex === 1) {
                    walkTexture = "sofia_walk";
                    walkAnim = "sofia_walk";
                } else if (charIndex === 2) {
                    walkTexture = "char2_walk";
                    walkAnim = "char2_walk";
                } else {
                    walkTexture = "mario_walk";
                    walkAnim = "mario_walk";
                }
                // Crear overlay de caminar
                const walkSprite = this.add
                    .sprite(sprite.x, sprite.y, walkTexture, 0)
                    .setDepth(sprite.depth + 1);
                ...
                try {
                    if (player.walkOverlay && player.walkOverlay.scene)
                        player.walkOverlay.destroy();
        ...
        if (punch && time - player.lastPunch > player.punchCD) {
            ...
            if (
                (charIndex === 0 &&
                    (this.textures.exists("char0_punch") ||
                        this.textures.exists("char0_trans"))) ||
                (charIndex === 1 &&
                    this.textures.exists("sofia_punch") &&
                    this.anims.exists("sofia_punch")) ||
                (charIndex === 2 &&
                    this.textures.exists("char2_punch") &&
                    this.anims.exists("char2_punch")) ||
                (charIndex === 3 &&
                    this.textures.exists("mario_punch") &&
                    this.anims.exists("mario_punch"))
            ) {
                ...
                if (charIndex === 0) {
                    ...
                    const useTrans =
                        ...
                        this.textures.exists("char0_trans") &&
                        this.anims.exists("char0_trans_punch");
                ...
                const punchSprite = this.add
                    .sprite(sprite.x, sprite.y, punchTexture, 1)
                    .setDepth(sprite.depth + 1);
                ...
                punchSprite.once("animationcomplete", () => {
                    ...
                    if (player.punchOverlay === punchSprite)
                        player.punchOverlay = null;
                    // Restaurar visibilidad del sprite base
                    try {
                        sprite.setAlpha(1);
                    } catch (e) {}
        ...
        if (
            shoot &&
            time - player.lastShot > player.shotCD &&
            this.getEnergy(player) >= 100
        ) {
            ...
            if (
                ...
                if (
                    charIndex === 1 &&
                    this.textures.exists("char1_shoot") &&
                    this.anims.exists("char1_shoot")
                ) {
                    const shootSprite = this.add
                        .sprite(sprite.x, sprite.y, "char1_shoot", 1)
                        .setDepth(sprite.depth + 1);
                    ...
                    shootSprite.once("animationcomplete", () => {
                        if (shootSprite && shootSprite.scene)
                            shootSprite.destroy();
                        if (player.shootOverlay === shootSprite)
                            player.shootOverlay = null;
                        // Restaurar visibilidad del sprite base
                        try {
                            sprite.setAlpha(1);
                        } catch (e) {}
                ...
                try {
                    this.cameras.main.flash(80, 120, 120, 120);
                } catch (e) {}
            }
        }
        ...
        if (
            ...
            const hasCharge =
                player.chargeOverlay && player.chargeOverlay.active;
            ...
            if (hasShoot) {
                ...
                if (hasWalk && player.walkOverlay) {
                    try {
                        player.walkOverlay.setAlpha(0);
                    } catch (e) {}
                }
                if (hasPunch && player.punchOverlay) {
                    try {
                        player.punchOverlay.setAlpha(0);
                    } catch (e) {}
                }
                if (hasBlock && player.blockOverlay) {
                    try {
                        player.blockOverlay.setAlpha(0);
                    } catch (e) {}
                }
                if (hasCharge && player.chargeOverlay) {
                    try {
                        player.chargeOverlay.setAlpha(0);
                    } catch (e) {}
                }
                ...
                if (hasBlock && player.blockOverlay) {
                    try {
                        player.blockOverlay.setAlpha(1);
                    } catch (e) {}
                }
                if (hasCharge && player.chargeOverlay) {
                    try {
                        player.chargeOverlay.setAlpha(1);
                    } catch (e) {}
                }
            } else if (hasPunch) {
                ...
                try {
                    player.punchOverlay.setAlpha(1);
                } catch (e) {}
            } else if (hasWalk) {
        ...
        if (action === "idle") {
            ...
            if (this.anims.exists(animKey)) {
                ...
                if (
                    (action === "punch" || action === "shoot") &&
                    player.actionLockUntil &&
                    time < player.actionLockUntil
                ) {
                    try {
    ...
    spawnProjectile(i) {
        ...
        if (
            projKey === "charles_bullet" &&
            this.anims.exists("charles_bullet_fly")
        ) {
            try {
                proj.play("charles_bullet_fly");
            } catch (e) {}
        } else if (
            projKey === "sofia_bullet" &&
            this.anims.exists("sofia_bullet_fly")
        ) {
            try {
                proj.play("sofia_bullet_fly");
            } catch (e) {}
        }
    ...
    doPunch(i) {
        ...
        if (this.anims.exists(atkPunchKey)) {
            try {
                attacker.sprite.anims.play(atkPunchKey, true);
            } catch (e) {}
        } else if (this.textures.exists(atkPunchKey)) {
            try {
                attacker.sprite.setTexture(atkPunchKey);
            ...
            if (this.bossActive && this.boss) {
                ...
                if (distBoss < hitRange) {
                    ...
                    const flash = this.add
                        .circle(b.x, b.y, 20, 0xff3300, 0.8)
                        .setDepth(10);
                    this.tweens.add({
                        targets: flash,
                        alpha: 0,
        ...
        if (dist < hitRange && !target.beingHit) {
            ...
            const isCharlesTrans =
                attacker.transformed &&
                ((i === 0 && this.player1Index === 0) ||
                    (i === 1 && this.player2Index === 0));
            if (!target.blocking) {
                ...
                if (isCharlesTrans) {
                    ...
                    const now = this.time.now;
                    if (
                        !target.blockingDamageEnd ||
                        now > target.blockingDamageEnd
                    ) {
    ...
    spawnFlyingEnemy() {
        ...
        enemy.healthBarBg = this.add
            .rectangle(enemy.x, enemy.y - 30, 50, 6, 0x000000)
            .setDepth(6);
        enemy.healthBar = this.add
            .rectangle(enemy.x, enemy.y - 30, 50, 6, 0xff0000)
            .setDepth(7);
    ...
    spawnGroundEnemy() {
        ...
        enemy.healthBarBg = this.add
            ...
            .setDepth(6);
        enemy.healthBar = this.add
            .rectangle(enemy.x, enemy.y - 30, 50, 6, 0x33cc33)
            .setDepth(7);
    ...
    spawnBoss() {
        ...
        const boss = this.physics.add.sprite(
            width * 0.5,
            height * 0.3,
            this.textures.exists("boss_walk") ? "boss_walk" : "char0_idle",
            0,
        );
        boss.setDepth(5);
        boss.setScale(1.8);
        ...
        this.physics.add.overlap(this.projectiles, boss, (proj) => {
            ...
            if (proj.parryReflected) {
                ...
                // Efecto visual
                const flash = this.add
                    .circle(proj.x, proj.y, 25, 0xff00ff, 0.8)
                    .setDepth(10);
                this.tweens.add({
                    targets: flash,
                    alpha: 0,
            ...
            if (this.boss && this.anims.exists("boss_attack")) {
                ...
                this.boss.once("animationcomplete", () => {
                    if (this.boss && this.anims.exists("boss_idle"))
        ...
        this.bossHealthBarBg = this.add
            .rectangle(barX, barY, barWidth, 30, 0x000000)
            .setDepth(100)
            .setScrollFactor(0);
        this.bossHealthBar = this.add
            .rectangle(barX, barY, barWidth, 30, 0xff0066)
            .setDepth(101)
            .setScrollFactor(0);
        this.bossHealthText = this.add
            .text(barX, barY, "JEFE: 1000/1000", {
                fontSize: "20px",
                color: "#ffffff",
                stroke: "#000000",
                strokeThickness: 3,
    ...
    spawnBossSpikeAt(x, y) {
        ...
        const spike = this.add
            .rectangle(x, y + 32, 20, 64, 0xcc0066)
            .setDepth(4);
        ...
        this.time.delayedCall(700, () => {
            try {
                if (spike && spike.scene) spike.destroy();
            } catch (e) {}
        });
    }
    ...
    updateBoss(time) {
        if (!this.bossActive || !this.boss || this.mode !== "cooperativo")
            return;
        const b = this.boss;
        if (!b || !b.scene) {
            this.bossActive = false;
        ...
        if (this.anims.exists("boss_move") && this.anims.exists("boss_idle")) {
            ...
            if (
                moving &&
                (!b.anims.isPlaying || b.anims.currentAnim.key !== "boss_move")
            ) {
                b.play("boss_move");
            } else if (
                !moving &&
                (!b.anims.isPlaying || b.anims.currentAnim.key !== "boss_idle")
            ) {
                b.play("boss_idle");
            }
        }
        ...
        if (this.bossHealthBar && this.bossHealthText) {
            ...
            this.bossHealthText.setText(
                `JEFE: ${Math.ceil(b.health)}/${b.maxHealth}`,
            );
        }
        ...
        if (time - b.lastAreaAttack > b.areaAttackCooldown) {
            ...
            const areaCircle = this.add
                .circle(b.x, b.y, 300, 0xff0066, 0.3)
                .setDepth(4);
            this.tweens.add({
                targets: areaCircle,
                alpha: 0,
        ...
        this.enemies.children.entries.forEach((enemy) => {
            ...
            if (enemy.healthBar && enemy.healthBarBg) {
                ...
                const healthPercent = Math.max(
                    0,
                    enemy.health / (enemy.maxHealth || 180),
                );
            ...
            if (enemy.isFlying) {
                ...
                const onFloor =
                    enemy.body &&
                    ((enemy.body.blocked && enemy.body.blocked.down) ||
                        (enemy.body.touching && enemy.body.touching.down));
                const horizDist = Math.abs(playerSprite.x - enemy.x);
                const shouldJump =
                    playerSprite.y + 20 < enemy.y ||
                    (enemy.body &&
                        enemy.body.blocked &&
                ...
                if (
                    onFloor &&
                    shouldJump &&
                    now - (enemy.lastJump || 0) > (enemy.jumpCooldown || 500) &&
                    horizDist < 160
                ) {
                ...
                const playerIframesUntil =
                    (targetP && targetP.groundMeleeIframesUntil) || 0;
                if (
                    dist < 65 &&
                    now - (enemy.lastMelee || 0) >
                        (enemy.meleeCooldown || 1400) &&
                    ...
                    try {
                        playerSprite.setVelocity(220 * kdir, -120);
                    } catch (e) {
                        /* ignore */
                    }
                }
    ...
    enemyShoot(enemy) {
        ...
        if (
            enemy.isFlying &&
            enemy.texture &&
            enemy.texture.key === "flying_enemy"
        ) {
        ...
        if (activeEnemies === 0 && this.waveActive) {
            ...
            if (this.currentWave < this.maxWaves) {
                this.waveText.setText(
                    `Oleada ${this.currentWave} completada. +150 HP. Siguiente en 3s...`,
                );
    ...
    handleCharlesSpecial(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX < -0.7 && !pad._specialLeft) {
                input = "L";
                pad._specialLeft = true;
            }
            if (axisX > 0.7 && !pad._specialRight) {
                input = "R";
                pad._specialRight = true;
            }
            if (axisX > -0.7 && axisX < 0.7) {
                pad._specialLeft = pad._specialRight = false;
            }
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._specialHit) {
                input = "X";
                pad._specialHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._specialHit = false;
        }
        ...
        if (input) {
            const now = time;
            if (
                player.specialBuffer.length === 0 ||
                now - player.specialBuffer[player.specialBuffer.length - 1].t <
                    1000
            ) {
                player.specialBuffer.push({ k: input, t: now });
                if (player.specialBuffer.length > 3)
                    player.specialBuffer.shift();
            } else {
            ...
            if (dist <= specialDist && !target.blocking) {
                ...
                if (charIdx === 0) {
                    ...
                    if (this.anims.exists(fireKey)) {
                        try {
                            sprite.anims.play(fireKey, false);
                        } catch (e) {}
                    } else if (this.textures.exists(fireKey)) {
                        try {
                            sprite.setTexture(fireKey);
                            sprite.setFrame(0);
                        } catch (e) {}
                    }
                }
    ...
    handleCharlesTransform(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._transRight) {
                input = "R";
                pad._transRight = true;
            }
            if (axisX < -0.7 && !pad._transLeft) {
                input = "L";
                pad._transLeft = true;
            }
            if (axisX > -0.7 && axisX < 0.7) {
                pad._transRight = false;
                pad._transLeft = false;
            }
            // Botón de golpe (X, index 2)
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._transHit) {
                input = "X";
                pad._transHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
        ...
        if (input) {
            const now = time;
            if (
                player.transformBuffer.length === 0 ||
                now -
                    player.transformBuffer[player.transformBuffer.length - 1]
                        .t <
                    1000
            ) {
                player.transformBuffer.push({ k: input, t: now });
                if (player.transformBuffer.length > 3)
                    player.transformBuffer.shift();
            } else {
    ...
    handleCharlesExplosion(i, time) {
        ...
        if (player.explosionPending && time > player.explosionTimer) {
            ...
            if (
                ...
                const grenadeY =
                    target.sprite.y +
                    (target.sprite.height ? target.sprite.height / 2 : 32);
                const grenade = this.add
                    .sprite(target.sprite.x, grenadeY, "charles_explosion", 0)
                    .setDepth(20);
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._explRight) {
                input = "R";
                pad._explRight = true;
            }
            if (axisX > -0.7 && axisX < 0.7) pad._explRight = false;
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._explHit) {
                input = "X";
                pad._explHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
        ...
        if (input) {
            const now = time;
            if (
                player.explosionBuffer.length === 0 ||
                now -
                    player.explosionBuffer[player.explosionBuffer.length - 1]
                        .t <
                    1000
            ) {
                player.explosionBuffer.push({ k: input, t: now });
                if (player.explosionBuffer.length > 3)
                    player.explosionBuffer.shift();
            } else {
        ...
        if (
            ...
            if (this.anims.exists(kickKey)) {
                try {
                    sprite.anims.play(kickKey, false);
                } catch (e) {}
            } else if (this.textures.exists(kickKey)) {
                try {
                    sprite.setTexture(kickKey);
                    sprite.setFrame(0);
                } catch (e) {}
            }
        }
    ...
    handleSofiaLaser(i, time) {
        ...
        if (
            (i === 0 && this.player1Index !== 1) ||
            (i === 1 && this.player2Index !== 1)
        )
            return;
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX < -0.7 && !pad._sofiaLeft) {
                input = "L";
                pad._sofiaLeft = true;
            }
            if (axisX > 0.7 && !pad._sofiaRight) {
                input = "R";
                pad._sofiaRight = true;
            }
            if (axisX > -0.7 && axisX < 0.7) {
                pad._sofiaLeft = pad._sofiaRight = false;
            }
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._sofiaHit) {
                input = "X";
                pad._sofiaHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
        ...
        if (input) {
            const now = time;
            if (
                player.sofiaLaserBuffer.length === 0 ||
                now -
                    player.sofiaLaserBuffer[player.sofiaLaserBuffer.length - 1]
                        .t <
                    1000
            ) {
                player.sofiaLaserBuffer.push({ k: input, t: now });
                if (player.sofiaLaserBuffer.length > 3)
                    player.sofiaLaserBuffer.shift();
            } else {
    ...
    handleSofiaTeleport(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._sofiaTRight) {
                input = "R";
                pad._sofiaTRight = true;
            }
            if (axisX < -0.7 && !pad._sofiaTLeft) {
                input = "L";
                pad._sofiaTLeft = true;
            }
            if (axisX > -0.7 && axisX < 0.7) {
                pad._sofiaTLeft = pad._sofiaTRight = false;
            }
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._sofiaTHit) {
                input = "X";
                pad._sofiaTHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._sofiaTHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.sofiaTeleportBuffer.length === 0 ||
                now -
                    player.sofiaTeleportBuffer[
                        player.sofiaTeleportBuffer.length - 1
                    ].t <
                    1000
            ) {
                player.sofiaTeleportBuffer.push({ k: input, t: now });
                if (player.sofiaTeleportBuffer.length > 3)
                    player.sofiaTeleportBuffer.shift();
            } else {
    ...
    handleSofiaMeteor(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._sofiaMRight) {
                input = "R";
                pad._sofiaMRight = true;
            }
            if (axisX > -0.7 && axisX < 0.7) pad._sofiaMRight = false;
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._sofiaMHit) {
                input = "X";
                pad._sofiaMHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._sofiaMHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.sofiaMeteorBuffer.length === 0 ||
                now -
                    player.sofiaMeteorBuffer[
                        player.sofiaMeteorBuffer.length - 1
                    ].t <
                    1000
            ) {
                player.sofiaMeteorBuffer.push({ k: input, t: now });
                if (player.sofiaMeteorBuffer.length > 3)
                    player.sofiaMeteorBuffer.shift();
            } else {
        ...
        if (
            ...
            try {
                ...
                if (castTex) {
                    ...
                    try {
                        if (sprite.setFrame) sprite.setFrame(7);
                    } catch (e) {
                        try {
                            sprite.setFrame(0);
                        } catch (ex) {}
            ...
            if (this.textures.exists("sofia_piedra")) {
                const tex = this.textures.get("sofia_piedra");
                const total =
                    tex && typeof tex.frameTotal === "number"
                        ? Math.max(1, tex.frameTotal)
                        : 1;
                ...
                try {
                    if (meteor && meteor.play) meteor.play("sofia_piedra_spin");
                } catch (e) {
                    /* ignore */
                }
            } else {
    ...
    handleSofiaRobo(i, time) {
        ...
        function check(buf) {
            return (
                buf &&
                buf.length === 3 &&
                ((buf[0].k === "L" && buf[1].k === "L" && buf[2].k === "X") ||
                    (buf[0].k === "R" && buf[1].k === "R" && buf[2].k === "X"))
            );
        }
        if (
            check(player.sofiaLaserBuffer) ||
            check(player.sofiaTeleportBuffer) ||
            check(player.sofiaMeteorBuffer)
        )
            roboTriggered = true;
        if (roboTriggered) {
            const charIdx = i === 0 ? this.player1Index : this.player2Index;
            ...
            if (this.anims.exists(roboKey)) {
                try {
                    sprite.anims.play(roboKey, false);
                } catch (e) {}
            } else if (this.textures.exists(roboKey)) {
                try {
                    sprite.setTexture(roboKey);
                    sprite.setFrame(0);
                } catch (e) {}
            }
            // clear buffers
    ...
    handleFranchescaEnergy(i, time) {
        ...
        if (
            (i === 0 && this.player1Index !== 2) ||
            (i === 1 && this.player2Index !== 2)
        )
            return;
        ...
        // Gamepad
        const pad = getPad(player.padIndex, this);
        if (pad && pad.connected && pad.buttons[2])
            punchHeld = punchHeld || pad.buttons[2].pressed;
        ...
        if (player.franchescaEnergyActive) {
            ...
            if (player.energy >= 100 * (this.game.loop.delta / 1000)) {
                player.energy = Math.max(
                    0,
                    player.energy - 100 * (this.game.loop.delta / 1000),
                );
                const target = this.players[1 - i];
                const dist = Phaser.Math.Distance.Between(
                    sprite.x,
                ...
                if (!player._energyCircle || !player._energyCircle.scene) {
                    player._energyCircle = this.add
                        .circle(sprite.x, sprite.y, 200, 0xff00cc, 0.13)
                        .setDepth(8);
                } else {
                ...
                if (
                    player.energy <= 0 ||
                    (player.franchescaEnergyDeactivateTime &&
                        time > player.franchescaEnergyDeactivateTime)
                ) {
                    player.franchescaEnergyActive = false;
                    player.franchescaEnergyDeactivateTime = null;
                    if (player._energyCircle && player._energyCircle.scene)
                        player._energyCircle.destroy();
                }
            } else {
                player.franchescaEnergyActive = false;
                player.franchescaEnergyDeactivateTime = null;
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX < -0.7 && !pad._franLeft) {
                input = "L";
                pad._franLeft = true;
            }
            if (axisX > 0.7 && !pad._franRight) {
                input = "R";
                pad._franRight = true;
            }
            if (axisX > -0.7 && axisX < 0.7) {
                pad._franLeft = pad._franRight = false;
            }
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._franHit) {
                input = "X";
                pad._franHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
        ...
        if (input) {
            const now = time;
            if (
                player.franchescaEnergyBuffer.length === 0 ||
                now -
                    player.franchescaEnergyBuffer[
                        player.franchescaEnergyBuffer.length - 1
                    ].t <
                    1000
            ) {
                player.franchescaEnergyBuffer.push({ k: input, t: now });
                if (player.franchescaEnergyBuffer.length > 3)
                    player.franchescaEnergyBuffer.shift();
            } else {
    ...
    handleFranchescaJumpSlash(i, time) {
        ...
        if (player.franchescaJumpPending) {
            ...
            if (
                ...
                if (!player.franAoeOverlay) {
                    const aoeSprite = this.add
                        .sprite(sprite.x, sprite.y, "fran_air_aoe", 0)
                        .setDepth(sprite.depth + 1);
            ...
            if (time > player.franchescaJumpTimer) {
                ...
                const slashCircle = this.add
                    .circle(sprite.x, sprite.y, 300, 0xff00cc, 0.18)
                    .setDepth(9);
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._franJumpRight) {
                input = "R";
                pad._franJumpRight = true;
            }
            if (axisX < -0.7 && !pad._franJumpLeft) {
                input = "L";
                pad._franJumpLeft = true;
            }
            if (axisX > -0.7 && axisX < 0.7) {
                pad._franJumpLeft = pad._franJumpRight = false;
            }
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._franJumpHit) {
                input = "X";
                pad._franJumpHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._franJumpHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.franchescaJumpBuffer.length === 0 ||
                now -
                    player.franchescaJumpBuffer[
                        player.franchescaJumpBuffer.length - 1
                    ].t <
                    1000
            ) {
                player.franchescaJumpBuffer.push({ k: input, t: now });
                if (player.franchescaJumpBuffer.length > 3)
                    player.franchescaJumpBuffer.shift();
            } else {
    ...
    handleFranchescaSteal(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._franStealRight) {
                input = "R";
                pad._franStealRight = true;
            }
            ...
            if (
                ...
                input = "X";
                pad._franStealHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._franStealHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.franchescaLaserBuffer.length === 0 ||
                now -
                    player.franchescaLaserBuffer[
                        player.franchescaLaserBuffer.length - 1
                    ].t <
                    1000
            ) {
                player.franchescaLaserBuffer.push({ k: input, t: now });
                if (player.franchescaLaserBuffer.length > 3)
                    player.franchescaLaserBuffer.shift();
            } else {
        ...
        const stealable = [];
        // Charles (0)
        if (this.player1Index === 0 || this.player2Index === 0) {
            /* not used */
        }
        // construir según target character
        const targetCharIndex =
            1 - i === 0 ? this.player1Index : this.player2Index;
    ...
    handleFranchescaUseStolen(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX < -0.7 && !pad._franUseLeft) {
                input = "L";
                pad._franUseLeft = true;
            }
            if (axisX > -0.7 && axisX < 0.7) pad._franUseLeft = false;
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._franUseHit) {
                input = "X";
                pad._franUseHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._franUseHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.franchescaUseBuffer.length === 0 ||
                now -
                    player.franchescaUseBuffer[
                        player.franchescaUseBuffer.length - 1
                    ].t <
                    1000
            ) {
                player.franchescaUseBuffer.push({ k: input, t: now });
                if (player.franchescaUseBuffer.length > 3)
                    player.franchescaUseBuffer.shift();
            } else {
                ...
                switch (ability.name) {
                    case "sofiaLaser":
                        ...
                        if (!target.blocking) {
                            ...
                            target.sprite.x =
                                sprite.x +
                                (target.sprite.x < sprite.x ? -offset : offset);
                            target.sprite.y = sprite.y;
                            target.sprite.setVelocity(0, 0);
                        }
                        break;
                    case "sofiaTeleport":
                        ...
                        // teleport thief near target and damage
                        {
                            ...
                            sprite.x =
                                target.sprite.x +
                                (sprite.x < target.sprite.x ? -offset : offset);
                            sprite.y = target.sprite.y;
                            sprite.setVelocity(0, 0);
                            if (!target.blocking)
                                target.health = Math.max(0, target.health - 30);
                    ...
                    default:
                        ...
                        if (!target.blocking)
                            target.health = Math.max(
                                0,
                                target.health - Math.floor(ability.cost / 2),
                            );
                        break;
                }
            }
    ...
    handleMarioBeam(i, time) {
        ...
        if (
            (i === 0 && this.player1Index !== 3) ||
            (i === 1 && this.player2Index !== 3)
        )
            return;
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX < -0.7 && !pad._marioLeft) {
                input = "L";
                pad._marioLeft = true;
            }
            if (axisX > 0.7 && !pad._marioRight) {
                input = "R";
                pad._marioRight = true;
            }
            if (axisX > -0.7 && axisX < 0.7)
                pad._marioLeft = pad._marioRight = false;
            if (pad.buttons[2] && pad.buttons[2].pressed && !pad._marioHit) {
                input = "X";
                pad._marioHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
        ...
        if (input) {
            const now = time;
            if (
                player.marioBeamBuffer.length === 0 ||
                now -
                    player.marioBeamBuffer[player.marioBeamBuffer.length - 1]
                        .t <
                    1000
            ) {
                player.marioBeamBuffer.push({ k: input, t: now });
                if (player.marioBeamBuffer.length > 3)
                    player.marioBeamBuffer.shift();
            } else {
        ...
        if (
            ...
            const laserSprite = this.add.sprite(
                sx,
                sy,
                "mario_laser_sangre",
                0,
            ...
            if (this.mode === "cooperativo") {
                ...
                if (this.enemies) {
                    this.enemies.children.entries.forEach((enemy) => {
                        ...
                        const inRange =
                            direction > 0
                                ? enemy.x > sx && enemy.x < sx + maxReach
                                : enemy.x < sx && enemy.x > sx - maxReach;
                ...
                if (this.bossActive && this.boss) {
                    ...
                    const inRange =
                        direction > 0
                            ? this.boss.x > sx && this.boss.x < sx + maxReach
                            : this.boss.x < sx && this.boss.x > sx - maxReach;
                    ...
                    if (inRange && distY < hitWidth) {
                        ...
                        // Flash visual
                        const flash = this.add
                            .circle(this.boss.x, this.boss.y, 30, 0xff3300, 0.8)
                            .setDepth(10);
                        this.tweens.add({
                            targets: flash,
                            alpha: 0,
                ...
                if (target && target.sprite) {
                    ...
                    if (inRange && distY < hitWidth) {
                        if (!target.blocking) {
                            ...
                            target.health = Math.max(
                                0,
                                target.health - Math.floor(baseDamage * 0.35),
                            );
                        }
                        target.beingHit = true;
                        target.hitTimer = this.time.now + 300;
    ...
    handleMarioExplosion(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._marioExplRight) {
                input = "R";
                pad._marioExplRight = true;
            }
            if (axisX < -0.7 && !pad._marioExplLeft) {
                input = "L";
                pad._marioExplLeft = true;
            }
            if (axisX > -0.7 && axisX < 0.7)
                pad._marioExplRight = pad._marioExplLeft = false;
            if (
                pad.buttons[2] &&
                pad.buttons[2].pressed &&
                !pad._marioExplHit
                ...
                input = "X";
                pad._marioExplHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._marioExplHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.marioExplBuffer.length === 0 ||
                now -
                    player.marioExplBuffer[player.marioExplBuffer.length - 1]
                        .t <
                    1000
            ) {
                player.marioExplBuffer.push({ k: input, t: now });
                if (player.marioExplBuffer.length > 3)
                    player.marioExplBuffer.shift();
            } else {
        ...
        if (
            ...
            if (
                ...
                const aguaY =
                    sprite.y + (sprite.height ? sprite.height / 2 : 32);
                const agua = this.add
                    .sprite(sprite.x, aguaY, "mario_agua", 0)
                    .setDepth(20);
            ...
            for (let j = 0; j < this.players.length; j++) {
                ...
                if (d <= RADIUS) {
                    ...
                    if (!target.blocking) {
                        ...
                        target.health = Math.max(
                            0,
                            target.health - Math.floor(DAMAGE * 0.35),
                        );
                    }
    ...
    handleMarioSmash(i, time) {
        ...
        if (pad && pad.connected) {
            const axisX = pad.axes.length > 0 ? pad.axes[0].getValue() : 0;
            if (axisX > 0.7 && !pad._marioSmashRight) {
                input = "R";
                pad._marioSmashRight = true;
            }
            ...
            if (
                ...
                input = "X";
                pad._marioSmashHit = true;
            }
            if (!(pad.buttons[2] && pad.buttons[2].pressed))
                pad._marioSmashHit = false;
        }

        if (input) {
            const now = time;
            if (
                player.marioSmashBuffer.length === 0 ||
                now -
                    player.marioSmashBuffer[player.marioSmashBuffer.length - 1]
                        .t <
                    1000
            ) {
                player.marioSmashBuffer.push({ k: input, t: now });
                if (player.marioSmashBuffer.length > 3)
                    player.marioSmashBuffer.shift();
            } else {
        ...
        if (
            ...
            if (
                ...
                const agua = this.add
                    .sprite(target.sprite.x, aguaStartY, "mario_bola_agua", 0)
                    .setDepth(20);
...
export class VictoryScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add
            ...
            .setOrigin(0.5);
        this.add
            .text(width / 2, 200, "¡Derrotaron al jefe!", {
                font: "36px Arial",
                color: "#ffffff",
            })
            .setOrigin(0.5);
        ...
        const restartTxt = this.add
            .text(restartBtn.x, restartBtn.y, "REINICIAR", {
                font: "24px Arial",
                color: "#00ffff",
            })
            .setOrigin(0.5);
        this.buttons.push({
            ...
            callback: () => {
                ...
                // Cambiar de escena directamente para no depender de timers tras detener la escena actual.
                this.scene.start("GameScene", {
                    player1Index: this.player1Index,
                    player2Index: this.player2Index,
                    mode: this.mode,
        ...
        const charSelTxt = this.add
            .text(charSelBtn.x, charSelBtn.y, "SELECCIONAR PERSONAJE", {
                font: "18px Arial",
                color: "#00ffff",
            })
            .setOrigin(0.5);
        ...
        // Gamepad support
        this.input.gamepad.on("connected", (pad) => {
            pad._leftPressed = pad._rightPressed = pad._aPressed = false;
        });
